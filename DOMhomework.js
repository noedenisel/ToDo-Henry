...
const elementoSpan = document.querySelector('#createdBy')
document.getElementById('createdBy').innerHTML = "Aplicación creada por NoeDenisel"
...
function buildToDo(ToDo, index) {
 ...
 toDoText.innerHTML = ToDo.description; //Utilizando el objeto toDo pasado como argumento, setear el 'toDoText' innerHTML  asignándole el valor de la propiedad 'description' del objeto ToDo.
 //toDoText.id = index //Asignarle como id del 'toDoText' el valor 'index' recibido como argumento

 if( ToDo.complete ) { //En función del atributo 'complete' del objeto ToDo recibido como argumento:
  //          - Si es true: asignarle a 'toDoText' la clase 'completeText'
  //          - Si es false: no asignarle ninguna clase
  toDoText.className = 'completeText' //todo.complete && todo.text.classname = "completeText"
  //document.getElementsByClassName("completeCheckbox").checked = true
 } 
...
function displayToDos() {
  ...
  for (i = 0 ; i < build.length ; i ++){ //Iterar sobre el resultado devuelto por la función 'buildToDos'
    toDoContainer.appendChild(build[i]) // ir agregndo cada elemento a 'toDoContainer'
  }
}
